...
def is_partial_tool_call(text: str) -> bool:
    strip = text.strip()
...
def parse_tool_calls(text: str, supported_functions: dict[str, FunctionDefinition] | None = None) -> tuple[
    list[ToolCall], bool]:
...
def get_arguments(function_block: str, expected_parameters: dict[str, Any] | None = None) -> tuple[str, bool]:
    ...
    for parameter_block in parameter_blocks:
        ...
        for param_name, param_tail in parameters:
            ...
            expected_type = expected_property.get(EXPECTED_PROPERTY_TYPE, 'string') if expected_property else 'string'
